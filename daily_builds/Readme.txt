...
/var/tmp/<имя_и_версия>.build. Туда загружается архив с исходниками KEDR 
...
Лучше в настройках целевых виртуальных машин в VirtualBox отключить 3D 
ускорение графики. VBoxHeadless (напр., в версии 4.1.22) может падать с 
segmentation fault, если 3D acceleration включено.
=======================================================================
...
К target-машинам сейчас система управления автоматичекий сборкой и 
проверкой KEDR обращается по SSH с использованием public key 
authentication, без пароля.

При добавлении новой target-машины в систему SSH-доступ к ней необходимо 
сконфигурировать, как описано ниже. 
...
[NB] Иногда этот шаг лучше выполнять на target-машине, см. п. 6 ниже.
...
5. SELinux на target-машине может влиять на ssh.
Например, из-за него может не читаться файл .ssh/authtorized_keys под root.

Если безпарольный доступ не работает, и есть причины подозревать в этом
SELinux, то проще всего отключить SELinux:

vi /etc/sysconfig/selinux

ищем переменную SELINUX, устанавливаем в disabled, перезагружаем машину.

6. Если безпарольный доступ не работает, и ssh -v выдает ошибку

"Agent admitted failure to sign using the key",

то это значит что target-машина не доверяет создателю ключа.

Обойти это проще всего создав ключи на target-машине(ssh-keygen), а затем
скопировав их на хост(например, через scp или sftp). Затем с хоста 
выполняются обычные действия начиная с пункта 2.
=======================================================================
