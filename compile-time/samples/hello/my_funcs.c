...
my_func_dummy_entry(void *func, unsigned int nargs, void **args)
{
	...
	printf("[DBG] Function entry, func = %p, nargs = %u, "
		"my_struct = %p, ret_addr = %p\n",
		func, nargs, p, __builtin_return_address(0));
	
	if (p == NULL) {
		printf("my_func_dummy_entry(): out of memory.\n");
		exit(EXIT_FAILURE);
	}
	
	p->func = func;
	p->pid = (unsigned long)getpid();
	
	for (i = 0; i < nargs; ++i) {
		printf("[DBG]\t#%u: 0x%lx", i, (unsigned long)args[i]);
		if (args[i]) {
			printf(" (value: 0x%lx)", *(unsigned long *)args[i]);
		}
		printf("\n");
	}
...
my_func_dummy_exit(struct my_struct *p)
{
	if (p) {
		printf(
		"[DBG] Function exit, func = %p, my_struct is at %p\n", 
			p->func, p);
	}
	else {
		printf("[DBG] Function exit, my_struct is at %p\n", p);
	}
...
{
	printf(
"[DBG] TID=%lu: memory %s at %p: accessed %u byte(s) starting from %p.\n",
...
{
	printf("[DBG] pre handler: vmalloc(%lu), pid: %lu\n", size, ls->pid);
...
my_func_vmalloc_post(void *ret, struct my_struct *ls)
{
	 ...
	 * to use. */
	printf("[DBG] post handler: vmalloc(%lu) returned %p.\n", 
		ls->data[0], ret);
}
...
{
	printf("[DBG] pre handler: vfree(%p)\n", addr);
...
{ /* [NB] The target function does not return value. */
	printf("[DBG] post handler: vfree(%p)\n",
		(void *)ls->data[0]);	
}
...
{
	printf("[DBG] pre handler: alloc_chrdev_region(%p, %u, %u, %s)\n",
...
{
	printf("[DBG] post handler: alloc_chrdev_region(%p, %u, %u, %s) = %d\n",
	       (void *)ls->data[0],
	       (unsigned)ls->data[1],
	       (unsigned)ls->data[2],
...
my_func_unregister_chrdev_region_pre(unsigned long long from, unsigned count, 
				     struct my_struct *ls)
{
	printf("[DBG] pre handler: unregister_chrdev_region(%lu, %u)\n",
...
{
	printf("[DBG] post handler: unregister_chrdev_region(%lu, %u)\n",
		ls->data[0], 
...
{
	printf("[DBG] pre handler: strlen(%s)\n", s);
...
{
	printf("[DBG] post handler: strlen(%s) = %lu\n",
...
my_func_memcpy_pre(void *dest, const void *src, unsigned long count, 
		   struct my_struct *ls)
{
	printf("[DBG] pre handler: memcpy(%p, %p, %lu)\n",
		dest, src, count);
...
{
	printf("[DBG] post handler: memcpy(%p, %p, %lu) = %p\n",
		(void *)ls->data[0],
...
my_func_memset_pre(void *s, int c, unsigned long count, 
		   struct my_struct *ls)
{
	printf("[DBG] pre handler: memset(%p, %d, %lu)\n",
		s, c, count);
	ls->data[0] = (unsigned long)s;
...
{
	printf("[DBG] post handler: memset(%p, %d, %lu) = %p\n",
		(void *)ls->data[0],
		(int)ls->data[1],
		ls->data[2],
		ret);
...
my_func_snprintf_pre(char *s, unsigned long n, const char *format, 
		     struct my_struct *ls)
{
	printf("[DBG] pre handler: snprintf(%p, %lu, '%s', ...)\n",
		s, n, format);
	ls->data[0] = (unsigned long)s;
	ls->data[1] = n;
...
{
	printf("[DBG] post handler: snprintf(%p, %lu, '%s', ...) = %d\n",
		(char *)ls->data[0],
		ls->data[1],
...
my_func_kmalloc_pre(unsigned long size, unsigned int flags, 
		     struct my_struct *ls)
{
	printf("[DBG] pre handler: kmalloc(%lu, %x)\n",
		size, flags);
	ls->data[0] = size;
	ls->data[1] = flags;
}
...
{
	printf("[DBG] post handler: kmalloc(%lu, %x) = %p\n",
		ls->data[0],
		(unsigned int)ls->data[1],
		ret);
}
...
{
	printf("[DBG] pre handler: kfree(%p)\n", addr);
	ls->data[0] = (unsigned long)addr;
}
...
{
	printf("[DBG] post handler: kfree(%p)\n", (void *)ls->data[0]);
}
			     ...
			     struct my_struct *ls)
{
	if (callee == (void *)vmalloc) {
		printf("[DBG] before indirect call to vmalloc(%lu)\n",
			arg0);
	}
	else {
		printf("[DBG] before indirect call, func=%p, args: {%lu}\n",
			callee, arg0);
	}
...
my_func_call_pvoid_ulong_post(void *ret, struct my_struct *ls)
{
	if (ls->callee == (void *)vmalloc) {
		printf("[DBG] after indirect call to vmalloc(%lu), ret=%p\n",
			ls->data[0], ret);
	}
	else {
		printf(
		"[DBG] after indirect call, func=%p, args: {%lu}, ret=%p\n",
...
my_func_call_void_pvoid_int_pre(void *arg0, int arg1, void *callee, 
				struct my_struct *ls)
{
	printf("[DBG] before indirect call, func=%p, args: {%p, %d}\n",
		callee, arg0, arg1);
...
{
	printf("[DBG] after indirect call, func=%p, args: {%p, %d}\n",
...
{
	printf("[DBG] before indirect call, func=%p\n", callee);
...
{
	printf("[DBG] after indirect call, func=%p, ret=%d\n",
		ls->callee, ret);
}
/* ====================================================================== */
...
"[DBG] Replacement for __kmalloc(%zu, %x) - simulating a failure.\n",
	...
	printf(
"[DBG] Replacement for __kmalloc(%zu, %x) - before the call, ID = %lu.\n",
		size, flags, ls->pid);
