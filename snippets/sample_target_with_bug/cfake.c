...
static void cfake_setup_cdevice(struct cfake_dev *dev, int index)
{
    ...
    dev->device_description = kmalloc(10, GFP_KERNEL);
    if(dev->device_description == NULL) return;
    
    snprintf(dev->device_description, 10, "Device %i", index);
	...
	if (cfake_devices) {
		for (i = 0; i < cfake_ndevices; ++i) {
			if (cfake_devices[i].dev_added)
			{
                ...
                kfree(cfake_devices[i].device_description);
			}
		}
		kfree(cfake_devices);
	}
