 ...
 * 
 * Determine, how real replacement will be performed.
 */
...
struct operation_replacer_data_layer
{
	...
	char* ops_mask_prev;
	/*
	 *  Replacement operations from the most nearest layer before this.
	 */
	char* ops_repl_prev;
};
...
{
	// Contains information about replacements for particuar object.
    data_map_t data_map;
...
ops_from_key_replace_pointer(void* key)
{
	...
	//pr_info("For key %p operations is pointed by %p.", key, result);
	return result;
}
...
operation_replacer_create_common(size_t elem_num, size_t ops_size)
{
    ...
    {
        pr_err("Cannot allocate replacement mask for non-special payloads.");
	...
	size_t ops_size)
{
    ...
    //pr_info("operation_replace_replace_pointer:");

    //pr_info("data: %p, key: %p.", data, key);
    //pr_info("repl: %p, mask: %p.", repl, mask);
    //pr_info("data->ops_orig : %p, data->ops: %p.", data_real->ops_orig, data_real->ops);
	...
	size_t ops_size)
{
	...
	{
		// Operations was changed from ones, set by replacer, to another.
...
{
	struct operation_data_at_place* data_real = data;
    pr_info("operation_get_orig_at_place: ops_orig is %p", data_real->ops_orig);
	return get_op_at_offset(data_real->ops_orig, op_offset);
}
	...
	size_t ops_size)
{
    ...
    FOR_EACH_OPERATION_IN_MASK(mask_op, mask, ops_size)
    {
        ...
        {
            pr_info("Operation at offset %d was changed outside of replacer.",
	...
	void* key)
{
    ...
    //pr_info("operation_replace was called");
    BUG_ON(atomic_read(&replacer->n_uses) == 0);
	...
	void* key)
{
	...
	if(IS_ERR(data))
	{
		 ...
		 * Only warning.
		 */
		pr_info("Attempt to restore operations at unexistent key %p.", key);
		return 1;
	}
	BUG_ON(data == NULL);
	...
	void* key)
{
	...
	if(IS_ERR(data))
	{
		 ...
		 * Only warning.
		 */
		pr_info("Attempt to clean operations replacement at unexistent key %p.", key);
		return 1;
	}
...
operation_replacer_data_alloc(size_t ops_size, int n_repl_layers)
{
	...
	if(data == NULL)
	{
		pr_err("Cannot allocate replacer's cache.");
		return NULL;
	}
    ...
    {
        pr_err("Cannot allocate 'repl' field for replacer's cache.");
        goto err_alloc_repl;
    }
    ...
    {
        pr_err("Cannot allocate 'mask' field for replacer's cache.");
        goto err_alloc_mask;
    }
    ...
    if(data->layers == NULL)
    {
        pr_err("Cannot allocate 'layers' field for replacer's cache.");
    ...
    for(i = 0; i < n_repl_layers; i++)
    {
        ...
        if(result)
        {
            pr_err("Cannot allocate layer for replacer's cache.");
...
static void operation_replacer_data_create(operation_replacer replacer)
{
	...
	if(data == NULL)
	{
		pr_err("Replacement facility will be disabled for this session.");
	...
	struct module *m)
{
	...
	//pr_info("Replacer %p become used now; number of replacement layers is %d.",
	//	replacer, replacer->n_repl_layers);
out:
	replacer->inside_callback = 0;
	...
	struct module *m, void undeleted_key(void* key))
{
	...
	mutex_unlock(&replacer->m);
	//pr_info("Replacer %p become unused now.", replacer);
out:
	replacer->inside_callback = 0;
}
	...
	struct operation_payload* payload)
{
	...
	if(atomic_read(&replacer->n_uses))
	{
		 ...
		 */
		pr_err("operation_payload_register_common: Attempt to register payload while target is being loaded.");
		return -EBUSY;
	}
	...
	void* key)
{
	 ...
	 */
	BUG_ON(IS_ERR(data));
	//pr_info("Original operations was requested for replacer %p and key %p.",
	//	replacer, key);
	BUG_ON(data == NULL);
