...
payload_special_module="./payload_special/payload_special.ko"
payload_special_module1="./payload_special1/payload_special1.ko"
...
was_intercepted_special1_file="/sys/module/payload_special1/parameters/was_intercepted"
...
{
    rmmod "$delegate_impl"
    rmmod "$payload_special_module"
    rmmod "$payload_module"
    rmmod "$payload_special_module1"
    rmmod "$base_module"
...
if ! insmod "$payload_special_module"; then
...
if ! insmod "$payload_special_module1"; then
    printf "Cannot load module with another special payload.\n"
    rmmod "$payload_module"
    rmmod "$payload_special_module"
    rmmod "$base_module"
    rmmod "$replacer_module"
...
if test "$was_intercepted_special" = "0"; then
    printf "Operation wasn't intercepted by special replacement.\n"
    unload_all
    exit 1
fi

was_intercepted_special1=`cat $was_intercepted_special1_file`
if test "$was_intercepted_special1" = "0"; then
    printf "Operation wasn't intercepted by the second special replacement.\n"
    unload_all
    exit 1
fi
...
if ! rmmod "$payload_special_module1"; then
    printf "Fail to unload second module with special payload.\n"
    exit 1
fi
...
if ! rmmod "$payload_special_module"; then
