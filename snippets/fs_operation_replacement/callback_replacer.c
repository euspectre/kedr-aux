...
callback_replacer callback_replacer_create(size_t elem_num)
{
    ...
    if(replacer == NULL)
    {
        pr_err("callback_replacer_create: Cannot allocate callback replacer.");
        return NULL;
    }
    replacer->callback_map = data_map_create(elem_num);
    if(replacer->callback_map == NULL)
    {
        pr_err("callback_replacer_create: Cannot create data map for callback replacer.");
        kfree(replacer);
        return NULL;
    }
...
void callback_replacer_destroy(callback_replacer replacer)
{
    ...
    if(replacer->payload)
    {
        pr_err("callback_replacer_destroy: Payload %p for "
            "callback replacer %p wasn't unregistered normally. "
            "Unregister it now.",
    ...
    struct module *m, void undeleted_key(void* key))
{
	...
	data_map_delete_all(replacer->callback_map, free_data, &undeleted_key_data);
    ...
    struct callback_payload* payload)
{
    ...
    if(replacer->used)
    {
        pr_info("callback_payload_register: Attempt to register payload while replacer is in using.");
        result = -EBUSY;
        goto out;
    }
    if(replacer->payload)
    {
        pr_info("callback_payload_register: Attempt to register payload while another payload is registered for the same callback.");
        result = -EBUSY;
        goto out;
    }
    ...
    struct callback_payload* payload)
{
    ...
    if(replacer->payload_used)
    {
        pr_info("callback_payload_unregister: Attempt to unregister payload while it is in using.");
        result = -EBUSY;
        goto out;
    }
    if(replacer->payload != payload)
    {
        pr_info("callback_payload_unregister: Attempt to unregister payload which wasn't registered.");
        result = -EINVAL;
        goto out;
    }
    replacer->payload = NULL;
