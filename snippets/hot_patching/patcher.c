...
repl___kmalloc(size_t size, gfp_t flags)
{
	...
	printk(KERN_INFO 
	"[hp_patcher] called __kmalloc(%u, %u): result = %p\n",
	(unsigned int)size, (unsigned int)flags, (void*)returnValue);
...
repl_kfree(void* p)
{
	...
	printk(KERN_INFO "[hp_patcher] called kfree(%p)\n", (void*)p);
...
repl_kmem_cache_alloc(struct kmem_cache* mc, gfp_t flags)
{
	...
	printk(KERN_INFO 
	"[hp_patcher] called kmem_cache_alloc(%p, %u): result = %p\n",
	(void*)mc, (unsigned int)flags, (void*)returnValue);
...
repl_kmem_cache_free(struct kmem_cache* mc, void* p)
{
	...
	printk(KERN_INFO 
	"[hp_patcher] called kmem_cache_free(%p, %p)", 
		(void*)mc, (void*)p);
